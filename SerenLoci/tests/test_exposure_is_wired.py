...
seren-loci's own main() calls it before the app is built, and that the
...
OPEN = 'server:\n  host: 0.0.0.0\n  port: 7422\n'
ALLOWED = 'server:\n  host: 0.0.0.0\n  port: 7422\n  allow_open_lan: true\n'
SCRUB = ("SEREN_LOCI_ALLOW_OPEN_LAN", "SEREN_LOCI_HOST", "SEREN_LOCI_PORT", "SEREN_LOCI_CONFIG",
         "SEREN_LOCI_BEARER_TOKEN", "SEREN_LOCI_BEARER_TOKEN_ENV", "SEREN_LOCI_BEARER_TOKEN_KEYRING",
         "SEREN_LOCI_TOKEN")
...
def _run(tmp_path, monkeypatch, yaml_text):
    ...
    holder = importlib.import_module("seren_loci.__main__")
    monkeypatch.setattr(holder, "create_app",
                        lambda *a, **k: (_ for _ in ()).throw(_AppBuilt()))
    monkeypatch.setattr(sys, "argv", ["seren_loci", "--config", str(cfg)])
    main_mod = importlib.import_module("seren_loci.__main__")
    main_mod.main()
...
def test_the_env_override_lets_the_same_config_through(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(sys, "argv", ["seren_loci", "--config", str(cfg)])
    with pytest.raises(_AppBuilt):
        importlib.import_module("seren_loci.__main__").main()
